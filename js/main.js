...
const uploadImage = async (e) => {
  ...
  if (headers.map(i => i.name).includes('iCCP')) {
    ...
    document.getElementById('result').innerText = 'iCCP Chunk absent.';
    addICCPHeader(indivBits, [], headers);
  }
}
...
const parsePNGImage = (bytes) => {
  ...
  while (currentPosition < bytes.length) {
    ...
    headers.push({
      ...
      endIndex: currentPosition + blockLength + 3,
      block: currentBlock
    });
...
const addICCPHeader = (byteArray, IccpHeader, allHeaders) => {
  ...
  let myByteStream = "";
  let iccpAdded = false;
  PNG_HEADER.forEach(i => {
    myByteStream += i;
  });
  allHeaders.forEach(i => {
    // add all headers, add ICCP IDAT
    if (i.name == 'IDAT' && !iccpAdded) {
      // add iccp header
      [
        "00",
        "00",
